...
sap.suite.ui.commons.ProcessFlowNode.prototype._setDimmedState = function () {
  ...
  if (bIsHighlighted || bIsSelected) {
    throw new Error("Cannot set dimmed state to highlighed or selected node " + this.getNodeId());
  }
...
sap.suite.ui.commons.ProcessFlowNode.prototype._adjustClassesForAggregation = function (oEvent) {
  ...
  var focusedStates = [sap.suite.ui.commons.ProcessFlowDisplayState.RegularFocused,
  ...
  var dimmedStates = [sap.suite.ui.commons.ProcessFlowDisplayState.DimmedFocused,
  ...
  function addAggregatedPressedClasses(that) {
    if (that._getZoomLevel() === sap.suite.ui.commons.ProcessFlowZoomLevel.Four) {
      ...
      if ((jQuery.inArray(that._getDisplayState(), dimmedStates)) >= 0) {
        ...
        that.$().addClass(that._nodeAggregatedPressedClassZoomLevel4);
      }
    } else {
      // If the node is dimmed
      if ((jQuery.inArray(that._getDisplayState(), dimmedStates)) >= 0) {
        ...
        that.$().addClass(that._nodeAggregatedPressedClass);
      }
    }
  }

  /**
   * Removes the CSS classes for pressed status (mouse up).
   */
  function removeAggregatedPressedClasses(that) {
    // If the node is focused
    if ((jQuery.inArray(that._getDisplayState(), focusedStates)) >= 0 && (that.$().hasClass(that._nodeAggregatedClass) || that.$().hasClass(that._nodeAggregatedClassZoomLevel4))) {
      if (that._getZoomLevel() === sap.suite.ui.commons.ProcessFlowZoomLevel.Four) {
    ...
    if (that.$().hasClass(that._nodeAggregatedPressedClass) || that.$().hasClass(that._nodeAggregatedPressedClassZoomLevel4)) {
      if (that._getZoomLevel() === sap.suite.ui.commons.ProcessFlowZoomLevel.Four) {
    ...
    // If the node is dimmed
    if (that.$().hasClass(that._nodeAggregatedDimmedPressedClass) || that.$().hasClass(that._nodeAggregatedDimmedPressedClassZoomLevel4)) {
      if (that._getZoomLevel() === sap.suite.ui.commons.ProcessFlowZoomLevel.Four) {
  ...
  /**
   * Adds the CSS classes for hovered status (mouseenter).
   */
  function addAggregatedHoveredClasses(that) {
    // If the node is focused
    if ((jQuery.inArray(that._getDisplayState(), focusedStates)) >= 0) {
      if (that._getZoomLevel() === sap.suite.ui.commons.ProcessFlowZoomLevel.Four) {
        that.$().removeClass(that._nodeAggregatedFocusedClassZoomLevel4).addClass(that._nodeAggregatedClassZoomLevel4);
      } else {
        that.$().removeClass(that._nodeAggregatedFocusedClass).addClass(that._nodeAggregatedClass);
      }
    }
    // If the node is dimmed
    if ((jQuery.inArray(that._getDisplayState(), dimmedStates)) >= 0) {
  ...
  /**
   * Removes the CSS classes for hovered status (mouseleave).
   */
  function removeAggregatedHoveredClasses(that) {
    // If the node is focused
    if ((jQuery.inArray(that._getDisplayState(), focusedStates)) >= 0 && (that.$().hasClass(that._nodeAggregatedClass) || that.$().hasClass(that._nodeAggregatedClassZoomLevel4))) {
      if (that._getZoomLevel() === sap.suite.ui.commons.ProcessFlowZoomLevel.Four) {
        that.$().removeClass(that._nodeAggregatedClassZoomLevel4).removeClass(that._nodeAggregatedPressedClassZoomLevel4).addClass(that._nodeAggregatedFocusedClassZoomLevel4);
      } else {
        that.$().removeClass(that._nodeAggregatedClass).removeClass(that._nodeAggregatedPressedClass).addClass(that._nodeAggregatedFocusedClass);
      }
    }
    // If the node is dimmed
    if ((jQuery.inArray(that._getDisplayState(), dimmedStates)) >= 0) {
      if (that._getZoomLevel() === sap.suite.ui.commons.ProcessFlowZoomLevel.Four) {
        that.$().removeClass(that._nodeAggregatedDimmedHoveredClassZoomLevel4).removeClass(that._nodeAggregatedDimmedPressedClassZoomLevel4).addClass(that._nodeAggregatedDimmedClassZoomLevel4);
      } else {
        that.$().removeClass(that._nodeAggregatedDimmedHoveredClass).removeClass(that._nodeAggregatedDimmedPressedClass).addClass(that._nodeAggregatedDimmedClass);
      }
    }
    // If the node is in regular state
    if (that.$().hasClass(that._nodeAggregatedPressedClass) || that.$().hasClass(that._nodeAggregatedPressedClassZoomLevel4)) {
...
sap.suite.ui.commons.ProcessFlowNode.prototype._getAriaText = function () {
  ...
  var aggregatedText = "";
  if (this.getType() === sap.suite.ui.commons.ProcessFlowNodeType.Aggregated) {
    aggregatedText = this._oResBundle.getText("PF_ARIA_TYPE");
  }

  var ariaValue = this._oResBundle.getText('PF_ARIA_NODE', [titleText, stateValueText, stateText, laneText, contentText, cParents, cChildren, aggregatedText]);
  return ariaValue;
};
...
/**
 * Returns a value that indicateds if the current node is dimmed or not.
 *
 * @private
