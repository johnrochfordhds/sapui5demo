	...
	sap.apf.modeler.core.ConfigurationObjects = function(inject) {
		...
		this.validateConfiguration = function(configuration) {
			var result = configuration && configuration.applicationTitle && configuration.applicationTitle.key && typeof configuration.applicationTitle.key === 'string' && configuration.steps && configuration.steps instanceof Array
					...
					&& configuration.bindings.length >= 0 && configuration.representationTypes && configuration.representationTypes instanceof Array && configuration.representationTypes.length >= 0 && configuration.facetFilters
					&& configuration.facetFilters instanceof Array && configuration.facetFilters.length >= 0 && configuration.categories && configuration.categories instanceof Array && configuration.categories.length >= 0;
			if (!result) {
				complain(11036);
			}
			return result;
