...
(function() {
	...
	sap.apf.core.ConfigurationFactory = function(oInject) {
		...
		var setItem = function(oItem) {
			...
			oInject.messageHandler.check((result === undefined), "Configuration includes duplicated identifiers (IDs): " + oItem.id + "", sap.apf.core.constants.message.code.errorCheckConfigurationWarning);
		};
		...
		function loadBinding(oBinding) {
			...
			oInject.messageHandler.check(oBinding.representations !== undefined && oBinding.representations instanceof Array !== false, "representations for binding " + oBinding.id + " not defined",
		...
		this.createRequest = function(request) {
			...
			if (typeof request === "string") {
				...
				oInject.messageHandler.check(oRequestConfig.type && oRequestConfig.type === "request" && oRequestConfig.service && oRequestConfig.entityType, 'Wrong request configuration when creating a new request');
		...
		if (oInject.constructor.registryProbe) {
			this.getRegistry = function() {
				var Probe = oInject.constructor.registryProbe;
				return new Probe(idRegistry);
			};
		}
	};
}());
