 ...
 * @param {sap.apf.utils.StartFilter} inject.constructor.StartFilter Start Filter constructor
...
sap.apf.utils.StartFilter = function(inject, config, context) {
	...
	var restriction = new sap.apf.core.utils.Filter(inject.instance.messageHandler);
	...
	this.setContext = function(internalFilter) {
		if(config.notConfigured === true){
			...
			inject.instance.messageHandler.putMessage(
				inject.instance.messageHandler.createMessageObject({
					code: '5100',
	...
	this.setSelectedValues = function(values) {
		...
		if (!initiallySelectedValues) {
			...
			determineSelected().done(function(selectedValues) {
				//Indicator that deserialize() was called - prevents that initiallySelectedValues promise is resolved by outdated done functions/callbacks. 
				if(initiallySelectedValues){
	...
	this.deserialize = function(serializedStartFilter) {
		if (serializedStartFilter.selectedValues && (serializedStartFilter.selectedValues.filters || serializedStartFilter.selectedValues.path)) {
