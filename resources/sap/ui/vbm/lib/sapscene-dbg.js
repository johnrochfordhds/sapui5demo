...
// the following does the killing and genesis of the scene................//
	...
	scene.Awake = function(target, mapmanager, maplayerstack) {
		...
		scene.m_MapDecoDiv = VBI.Utilities.CreateGeoSceneDivCSS(target + "-geoscene-decolayer", "");
		scene.m_Div.appendChild(scene.m_MapDecoDiv);

		// add a layer div for legend windows
		scene.m_LegendLayerDiv = VBI.Utilities.CreateGeoSceneDivCSS(target + "-geoscene-legendlayer", "vbi-window-layer");
		scene.m_Div.appendChild(scene.m_LegendLayerDiv);
		
		// add a layer div for VBI detail window elements
		scene.m_WindowLayerDiv = VBI.Utilities.CreateGeoSceneDivCSS(target + "-geoscene-winlayer", "vbi-window-layer");
		if (VBI.m_bIsPhone) {
