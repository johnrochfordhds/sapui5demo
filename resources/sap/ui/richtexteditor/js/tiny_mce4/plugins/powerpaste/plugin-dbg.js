...
define(
  ...
  function(Arr, Element, Attr, document) {
    ...
    var restoreStyleAttrs = function (node) {
      Arr.each(Arr.map(node.getElementsByTagName('*'), Element.fromDom), function (elm) {
        if (Attr.has(elm, 'data-mce-style') && !Attr.has(elm, 'style')) {
          Attr.set(elm, 'style', Attr.get(elm, 'data-mce-style'));
        }
      })
    };
    ...
    return {
      ...
      restoreStyleAttrs: restoreStyleAttrs
    };
  }
);
...
define(
  ...
  function (Validate) {
     ...
     * Any template fields of the form richtexteditor are replaced by the string or number specified as obj["name"]
...
define(
  ...
  function (Arr, Id, PlatformDetection, ImageAsset, Future, FileReader, URL, Struct) {
    var blobStruct = Struct.immutable('id', 'obj', 'objurl');
    var format = Struct.immutable('blob', 'base64');
     ...
     * @param preprocessor do somthing to the blob before, returns a future
     * @param callback the callback function for the {ephox.hermes.asset.ImageAsset[]}
     */
    var fromBlobs = function (blobs, preprocessor, callback) {
      ...
      var readBlob = function (blob) {
        return Future.nu(function (callback) {
          ...
          fr.onload = function(e) {
            callback(format(blob, e.target));
          };
          fr.readAsDataURL(blob);
        });
      };

      Future.mapM(blobs, function (blob) {
        return preprocessor(blob).bind(readBlob);
      }).get(function (formats) {
        Arr.each(formats, function (format) {
          var b = blob(format.blob());
          var asset = ImageAsset.blob(b.id(), format.blob(), b.objurl(), format.base64());
          addAsset(asset);
        });
        callback(processed);
      });
     ...
     * @param preprocessor do somthing to the file before, returns a future
     * @param callback the callback function for the {ephox.hermes.asset.ImageAsset[]}
     */

    var toAssets = function (files, preprocessor, callback) {
      fromBlobs(files, preprocessor, callback);
    };
     ...
     * @param preprocessor do somthing to the image before, returns a future
     * @param callback, when the future resolves return the value via callback
     * @return {ephox.hermes.asset.ImageAsset[]} mapped image assets
     */
    var fromImages = function (images, preprocessor, callback) {
      Future.mapM(images, function (image) {
        return preprocessor(image).map(function (img) {
...
define(
  ...
  function (ImageExtract) {
     ...
     */
    var toAssets = function (files, preprocessor, callback) {
      return ImageExtract.toAssets(files, preprocessor, callback);
    };
     ...
     */
    var fromBlobs = function (blobs, preprocessor, callback) {
      return ImageExtract.fromBlobs(blobs, preprocessor, callback);
    };
...
define(
  ...
  function (Arr, Struct, console) {
    var fields = [ 'officeStyles', 'htmlStyles', 'isWord', 'proxyBin', 'isInternal' ];
...
(function (define, require, demand) {
define(
  'ephox.bowerbird.api.BlobResult',
  ...
  function (Merger, ADT) {
    ...
    var cata = function (blobResult, none, error, blob) {
      return blobResult.fold(none, error, blob);
    };
...
(function (define, require, demand) {
define(
  'ephox.bowerbird.codes.Base64ToBlob',
  ...
  function (BlobResult, Blob, Uint8Array, Window, Option, Result, Strings, Array, Math) {
    var toBlob = function (base64, mimetype) {
    ...
    var isBase64 = function (rawsrc) {
      // ignore all images that aren't data URI, and of those ignore anything that isn't a base64 image
      return Strings.startsWith(rawsrc, 'data:image/') && rawsrc.indexOf(';base64,') > 'data:image/'.length;
    };

    var convert = function (rawsrc) {
      if (!isBase64(rawsrc)) {
        return BlobResult.none();
      } else {
        var endMime = rawsrc.indexOf(';');
        // mimetype is between the 'data:' and the semicolon
        var mimetype = rawsrc.substr('data:'.length, endMime - 'data:'.length);

        // base64 data begins after 'base64,'
        var base64 = rawsrc.substr(endMime + ';base64,'.length);
        try {
          var r = BlobResult.blob(toBlob(base64, mimetype));
          return r;
        } catch (e) {
          return BlobResult.error(e);
        }
      }
    };
...
define(
  ...
  function (Base64ToBlob) {
    ...
    /**
     * Returns a BlobResult.
     *
     * data BlobResult = none | error String | blob Blob
     */
    var toBlob = function (src) {
      return Base64ToBlob.convert(src);
    };
...
define(
  ...
  function (Base64, BlobResult, FutureStep, PasteResponse, Arr, ImageAsset, ImageExtract, Fun, Option, Struct, Attr, Element, InsertAll, SelectorFilter) {
    var blobImage = Struct.immutable('blob', 'image');

    var toBlobImage = function (image, src) {
      // we have a base 64
      var blob = Base64.toBlob(src);
      return BlobResult.cata(blob, Option.none, Option.none, function (b) {
        // I can't set the source here, because the image URL is created by asset generation.
        return Option.some(blobImage(b, image));
      });
    };

    var isBase64 = function (src) {
      return src.indexOf('data:image') -1 && src.indexOf('base64') > -1;
    };

    var scan = function (elements) {
      var container = Element.fromTag('div');
      InsertAll.append(container, elements);
      var images = SelectorFilter.descendants(container, 'img[src]');
      return Arr.bind(images, function (image) {
        var src = Attr.get(image, 'src');
        return isBase64(src) ? toBlobImage(image, src).toArray() : [ ];
      });
    };

    var updateSources = function (scanned, assets) {
      Arr.each(assets, function (asset, i) {
        // Assumption: the indices are in sync with the scanned images.
        ImageAsset.cata(asset, function (id, blob, objurl, data) {
          var target = scanned[i].image();
          Attr.set(target, 'src', objurl);
        }, Fun.noop);
      });
    };
    ...
    return function (preprocessor) {
      return function (raw, result) {
        return FutureStep.sync(function (callback) {
          ...
          var scanForAssets = function (elements) {
            var scanned = scan(elements);
            var scannedBlobs = Arr.map(scanned, function (s) { return s.blob(); });
            ImageExtract.fromBlobs(scannedBlobs, preprocessor, function (assets) {
              updateSources(scanned, assets);
              FutureStep.call(callback, {
          ...
          // When there are no assets, scan in the elements to see if there are any images that are base64.
          // If so, preprocess them and make them assets.
          var onPaste = function (elements, assets, _message) {
            return assets.length === 0 ? scanForAssets(elements) : passthrough();
          };

          // NOTE: Like Flash, this does not have any special handling for incomplete.
...
define(
  ...
  function (Rtf, Arr, ImageExtract, Fun, Option, Attr, Class, console) {
    ...
    var convert = function (images, imageData, preprocessor, callback, translations) {
      ...
      ImageExtract.fromBlobs(blobs, preprocessor, function (assets) {
        Arr.each(assets, function (asset, i) {
          asset.fold(function (id, blob, objurl, data) {
    ...
    return function (makeFlash, cementConfig) {
      ...
      var gordon = function (content, baseAssets) {
        var response = function (event) {
          ...
          var preprocessor = cementConfig.preprocessor;

          Correlation.convert(images, imageData, preprocessor, function (assets, failedImage) {
            if (failedImage) {
        ...
        if (cementConfig.allowLocalImages === true && cementConfig.enableFlashImport === true) {
...
define(
  ...
  function (FutureStep, PasteResponse, console) {
...
define(
  ...
  function (FutureStep, Bundle, PasteResponse, Arr, ImageAsset, ImageExtract, Attr, Element, console) {
    ...
    var handle = function (items, preprocessor) {
      ...
      return FutureStep.sync(function (callback) {
        ImageExtract.toAssets(files, preprocessor, function (assets) {
...
define(
  ...
  function (Unicode, Arr, Obj, Option, Attr, Css, Element, Html, Insert, InsertAll, Node, Remove, Text, Traverse) {
    ...
    var findListItemBefore = function (element) {
      return Traverse.prevSibling(element).bind(function (sibling) {
        if (Node.isText(sibling) && Unicode.trimNative(Text.get(sibling)).length === 0) return findListItemBefore(sibling);
        return Node.name(sibling) === 'li' ? Option.some(sibling) : Option.none();
      });
    };

    var properlyNest = function (element) {
      // This is trying to make lists that have ol/ul as direct children of other ol/ul 
      // valid HTML by putting the nested list inside a li.
      Traverse.parent(element).each(function (parent) {
        var tag = Node.name(parent);
        if (Arr.contains([ 'ol', 'ul' ], tag)) {
          // We are within an incorrectly nested list. Firstly, see if there are any prior siblings
          // which are list items. If there are, add the nested list to that sibling. Otherwise, wrap it.
          findListItemBefore(element).fold(function () {
            var li = Element.fromTag('li');
            Css.set(li, 'list-style-type', 'none');
            Insert.wrap(element, li);
          }, function (prior) {            
            Insert.append(prior, element);
          });
        }
      });
    };
...
define(
  ...
  function (FutureStep, Bundle, PasteResponse, PlatformDetection, RunPaste, Result, Elements, console) {
    ...
    var transfer = function (doc, container, isWord, merging) {
      ...
      return elements.fold(function (message) {
        ...
        return FutureStep.sync(function (callback) {
          FutureStep.call(callback, {
            response: PasteResponse.paste(elems, [ ]),
            bundle: Bundle.nu({})
          });
        });
      });
...
define(
  ...
  function (PasteTransfer, Arr, Future, Option, Remove, Traverse) {
    var external = function (doc, proxyBin, merging, isWord) {
      return PasteTransfer.transfer(doc, proxyBin, isWord, merging);
    };
      ...
      return function (raw, result) {
        ...
        return Bundles.proxyBin(bundle).handle(
          ...
          function (proxyBin) {
            ...
            return isInternal ? OffscreenHandler.internal(doc, intraFlag, proxyBin) : OffscreenHandler.external(doc, proxyBin, merging, isWord);
          }
        );
      };
    };

    var noImages = function (_preprocessor) {
      return function (_info, _result) {
        return FutureStep.error('errors.local.images.disallowed');
      };
    };

    // Image pasting is only used in Webkit, and is forbidden in Safari.
    var images = function (preprocessor) {
      return function (items, _result) {
        if (platform.browser.isSafari()) return FutureStep.error('safari.imagepaste');
        else return ImageHandler.handle(items, preprocessor);
      };
    };
...
define(
  ...
  function (FutureStep, Bundle, CementConstants, PlatformDetection, Sources, Class, Traverse) {
    ...
    return function (makeProxy, prePasteFilter, body, intraFlag) {
      return function (raw, result) {
...
define(
  ...
  function (PasteProcesses, Inspection, Base64Step, FlashStep, MergeStep, PasteSteps, ProxyStep, PlatformDetection, Fun, Option, Strings, window) {
    var platform = PlatformDetection.detect();
    ...
    var internal = function (prePasteFilter, makeProxy, body, intraFlag) {
      ...
      return strategy('Within Textbox.io (tables) pasting', getAvailable, [
        PasteProcesses.normal(ProxyStep(makeProxy, prePasteFilter, body, intraFlag)),
    ...
    var pastiche = function (createDialog, config, prePasteFilter, makeProxy, body, makeFlash, intraFlag) {
      return strategy('Outside of Textbox.io pasting (could be internal but another browser)', Option.some, [
        ...
        PasteProcesses.normal(Base64Step(config.preprocessor))
      ], false);
    };
    ...
    var image = function (config) {
      return strategy('Image pasting', getImage, [
        PasteProcesses.normal(config.allowLocalImages === false ? PasteSteps.noImages(Fun.noop) : PasteSteps.images(config.preprocessor))
      ], true);
    };
...
define(
  ...
  function (PlatformDetection, Globaliser, Fun, Thunk, Option, Writer, Body, Css, DomEvent, Element, Insert, Ready, Remove) {
    ...
    var detection = PlatformDetection.detect();
    var browser = detection.browser;

    var unsupported = (
      // IE doesn't give us raw HTML so there's not point.
      // In addition, IE11 can't handle loading GWT in an iframe (possible due to the setting that collapses into a single JS file).
      // It seems to load ok in WordTest.html, though, - worth looking into if we ever need GWT in IE.
      browser.isIE() ||

      // We don't support word import on mobile, so let's not download 60kb of pointless JavaScript okay?
      detection.deviceType.isiOS() || detection.deviceType.isAndroid()
    );

    // TODO: Unsupported platforms will show a 'word import not ready' error if they happen to suddenly give us office HTML on the clipboard.
    // This is less than ideal.
    var load = unsupported ? Fun.noop : Thunk.cached(function (baseUrl) {
      var container = Element.fromTag('div');
...
define(
  ...
  function (Cement, Arr, Fun, I18n, Defaults, Styles, ErrorDialog, ModernTinyDialog, NodeUtil, Future, DomEvent, Element, setTimeout, tinymce) {
    return function (editor, url, settings, uploader) {
      ...
      editor.on('init', function(e) {
        ...
        var cementSettings = {
          ...
          allowLocalImages: editor.settings.powerpaste_allow_local_images !== false,
          enableFlashImport: editor.settings.powerpaste_enable_flash_import !== false,
          preprocessor: function (b) {
            return Future.pure(b);
          }
        };
        ...
        cement.events.insert.bind(function(event) {
          ...
          setTimeout(function(){
            ...
            editor.undoManager.transact(function(){
              //Content insertion
              editor.insertContent(stringHTML, {merge: editor.settings.paste_merge_formats !== false});
              NodeUtil.restoreStyleAttrs(editor.getBody());
